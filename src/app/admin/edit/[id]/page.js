...
export default function EditPerson() {
  ...
  const fetchPerson = async () => {
    try {
      ...
      if (result.success) {
        ...
        // Extract assistanceTypeIds
        let aIds = [];
        if (p.assistances && p.assistances.length > 0) {
           aIds = p.assistances.map(a => String(a.assistanceTypeId));
        }
        ...
        setFormData({
          ...
          gnDivisionId: p.gnDivisionId ? String(p.gnDivisionId) : '',
          address: p.address || '',
          disabilityCategoryId: p.disabilityCategoryId ? String(p.disabilityCategoryId) : '',
          disabilityTypeId: p.disabilityTypeId ? String(p.disabilityTypeId) : '',
          occupation: p.occupation || '',
          monthlyIncome: p.monthlyIncome ? String(p.monthlyIncome) : '',
          assistanceReceived: p.assistanceReceived ? JSON.parse(p.assistanceReceived) : [],
          assistanceTypeIds: aIds,
          familyMembers: p.familyMembers || [],
          equipments: p.equipments || [],
          profilePhotoBase64: p.profilePhotoBase64 || '',
          nameWithInitials: p.nameWithInitials || '',
          altContactNumber: p.altContactNumber || '',
          villageId: p.villageId ? String(p.villageId) : '',
          disabilityPercentage: p.disabilityPercentage ? String(p.disabilityPercentage) : '',
          causeOfDisability: p.causeOfDisability || '',
          disabilityStartDate: p.disabilityStartDate ? new Date(p.disabilityStartDate).toISOString().split('T')[0] : '',
          educationLevelId: p.educationLevelId ? String(p.educationLevelId) : '',
          schoolAttended: p.schoolAttended || '',
          employmentStatusId: p.employmentStatusId ? String(p.employmentStatusId) : '',
          headOfFamily: p.headOfFamily || '',
          familyMembersCount: p.familyMembersCount ? String(p.familyMembersCount) : '',
          guardianName: p.guardianName || '',
          guardianContact: p.guardianContact || '',
          hospital: p.hospital || '',
          doctor: p.doctor || '',
          medicalCertificateNumber: p.medicalCertificateNumber || '',
          medicalCertificateExpiry: p.medicalCertificateExpiry ? new Date(p.medicalCertificateExpiry).toISOString().split('T')[0] : '',
          notes: p.notes || ''
        });
      }
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/persons/${id}`, { method: 'PUT',
  ...
  if (dataLoading) return <div style={{padding: '2rem', textAlign: 'center'}}>தரவுகள் ஏற்றப்படுகின்றன...</div>;
      ...
      <form onSubmit={handleSubmit} className="card">
        ...
        <div style={{ textAlign: 'center', marginTop: '3rem' }}>
          <button type="submit" className="btn btn-primary" disabled={loading} style={{ padding: '1rem 3rem', fontSize: '1.2rem', width: '100%', maxWidth: '400px' }}>
            {loading ? 'சேமிக்கப்படுகிறது...' : 'மாற்றங்களைச் சேமிக்க'}
          </button>
        </div>
      </form>
    </div>
